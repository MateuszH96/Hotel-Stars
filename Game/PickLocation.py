...
class PickLocation:
    def __init__(self, window: Window,names):
        ...
        self.__names = names
        self.__player = 0
        self.__font = pg.font.SysFont(VG.FONT_NAME_LOCATION, VG.FONT_SIZE_LOCATION)
    ...
    def __createObjects(self):
        ...
        self.__locationToChoose = pg.transform.scale(self.__locationToChoose,(VG.SIZE))
        self.__staticText = self.__font.render("Teraz wybiera", True, VG.FONT_COLOR_LOCATION)
        self.__pickLocation = Btn("OK.jpg",VG.OK_COORDINATE_X,VG.OK_COORDINATE_Y,VG.OK_WIDTH,VG.OK_HEIGHT)
    
    def __createListObject(self):
        dynamicText = self.__font.render(self.__names[self.__player], True, VG.FONT_COLOR_LOCATION)
        dynamicTextPosition = (VG.DYNAMIC_TEXT_COORDINATE_X,VG.DYNAMIC_TEXT_COORDINATE_Y)
        listOfObjects = []
        staticTextCoordinates = (VG.STATIC_TEXT_COORDINATE_X,VG.STATIC_TEXT_COORDINATE_Y)
        listOfObjects.append([self.__staticText,staticTextCoordinates])
        listOfObjects.append([self.__pickLocation.getImage(),(self.__pickLocation.getX(),self.__pickLocation.getY())])
